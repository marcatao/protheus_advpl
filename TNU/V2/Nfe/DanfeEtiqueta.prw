...
user function ImpDfEtq(cUrl, cIdEnt, lUsaColab)
    ...
    /*---------------------------------------*/
    // MELHORIA ALUMBRA IMPRESSAO VOLUME      / 
    /*------------------DEO--20230903--------*/
    if nTipImp == 1 .OR. nTipImp == 3   //ALUMBRA DEO 20230903 
        if empty(cLocImp)
        ...
        if (!empty(cProtocolo) .or. !empty(cDpecProt))
            ...
            if nTipoDoc == 2
                ...
                if lSeek 
                    ...
                    cLoja := SF1->F1_LOJA
                    cTransp    := Posicione("SA4",1,xFilial("SA4")+SF1->F1_TRANSP,"A4_NOME")
                endif
            endif
            ...
            aNFe := array(10) //ALUMBRA DEO 20230903
            aNfe[01] := SF3->F3_CHVNFE
            aNfe[02] := cProtocolo
            aNfe[03] := cDpecProt
            aNfe[04] := cLogo
            aNfe[05] := if( SubStr(SF3->F3_CFO,1,1) >= '5', "1", "0" ) // 0 - Entrada / 1 - Saída
            aNfe[06] := SF3->F3_NFISCAL
            aNfe[07] := SF3->F3_SERIE
            aNfe[08] := SF3->F3_EMISSAO
            aNfe[09] := cTotNota
            aNfe[10] := cTransp  //ALUMBRA DEO 20230903

            aDest := array(5)  //ALUMBRA DEO 20230903
            if ( SubStr(SF3->F3_CFO,1,1) < '5' .and. SF3->F3_TIPO $ "DB") .or. (SubStr(SF3->F3_CFO,1,1) >= '5' .and. !SF3->F3_TIPO $ "DB")
                if SA1->(dbSeek(xFilial("SA1")+SF3->F3_CLIEFOR+SF3->F3_LOJA))
                    ...
                    cInscr := SA1->A1_INSCR
                    cUF := SA1->A1_EST
                    cMun := SA1->A1_MUN  //ALUMBRA DEO 20230903
                endif
                ...
                if SA2->(dbSeek(xFilial("SA2")+SF3->F3_CLIEFOR+SF3->F3_LOJA))
                    ...
                    cInscr := SA2->A2_INSCR
                    cUF := SA2->A2_EST
                    cMun := SA2->A2_MUN  //ALUMBRA DEO 20230903
                endif
            endif
            ...
            aDest[4] := alltrim(cUF)
            aDest[5] := alltrim(cMun) //ALUMBRA DEO 20230903
    ...
    if lOk
        if nTipImp == 1 .OR. nTipImp == 3 // 1 - Térmica 
            MSCBCLOSEPRINTER()
        elseif nTipImp == 2 // 2 - Normal
            if nContDanfe <> 6
                oPrinter:EndPage()
...
static function impZebra(aNFe, aEmit, aDest, nTipImp)
    ...
    Local nI         := 0 //ALUMBRA DEO 20230903
    Local nVol       := 0
    Local nCodVol    := 1
    Local nPedido    := 2
    Local nNumPc     := 3
...
cQuery += "      AND DCV_SEQUEN = C9_SEQUEN "
cQuery += "      AND SC9.D_E_L_E_T_ = '' "
cQuery += "      AND C9_NFISCAL = " + cNF + " "
cQuery += "    JOIN " + RetSqlName("SC6") + " SC6 ON C6_FILIAL = C9_FILIAL "
